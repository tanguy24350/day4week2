...
def handle_count
  ...
  puts "Il y en a #{Arr.count}"
end

def handle_short
    puts "Quel est le handle le plus court de cette liste ?"
    puts "Le handle le plus court est #{Arr.sort_by(&:length)[0]}"
end

def handle_5carac
  puts "Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)"
  puts "Il y en a #{Arr.count{|string| string.length == 6}} de 5 caractères"
end

def handle_majuscule
  puts "Combien commencent par une majuscule (première lettre juste après le @) ?"
  puts "Il y en a #{Arr.select {|e| e =~ /^@[A-Z]/}.size}"
end

def handle_alpha
  Arr.map!(&:upcase)
  Arr.each {|replace| replace.gsub!(/_/, "")}
...
def handle_loop
    ...
    h.each {|key, value| puts "Il y a #{value} handles qui ont #{key} caracteres"}
end



def main
    puts "Quelle question veut tu entre 1 et 9 (9 pour quitter) ?"
